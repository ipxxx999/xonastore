...
function update_license($license) {
	$license = trim(strip_tags($license));
	$medb = get_connect_db();
    $row = $medb->query("UPDATE options SET license = '$license'");
    $medb->disconnect();
	return $row;
}
...
$licencia = trim(strip_tags($_POST['licencia']));
$check = isValidMd5($licencia);
if($check) {
update_license($licencia);	
}
...
if (user::isLoggedIn()){	
$licencia = consultar_licencia();
$licencia = $licencia['license'];
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL,"ajax.php");
curl_setopt($ch, CURLOPT_POST, 1);
curl_setopt($ch, CURLOPT_POSTFIELDS,"tipo=verificar_licencia&licencia=".$licencia."&postvar3=value3");
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
$server_output = curl_exec($ch);
curl_close ($ch);
if($server_output == 'error') {	
$titulo_process = 'Licencia Invalida';
$mensaje_process = 'Debe Verificar la licencia para acceder al Panel.';
require(ABSVIEWS . '/verify-license.php');
exit();
}
...
function consultar_licencia() {
	...
	$row = $medb->select("SELECT license FROM options", true);
	return $row;
}
...
$medb = get_connect_db();
$row = $medb->select("SELECT * FROM links WHERE status = 0 AND error = '' LIMIT 5", false);
$medb->disconnect();
if(empty($row)){
return array();
}
...
if (!in_array("480p", $data) && $calidad == '480p') {
    $data[] = '480p';
}
...
$medb = get_connect_db();	
$url = 'http://10.0.8.4/xonaplay/version.php?tipo=consulta';
